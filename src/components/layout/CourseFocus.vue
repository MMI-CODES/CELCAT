		...
		<div class="bg-white font-semibold shadow-2xl p-6 space-y-4 max-md:h-screen md:rounded-3xl md:min-w-1/2 md:p-8 lg:min-w-1/3 dark:bg-slate-900" :style="{ backgroundColor: (color[isDark() ? 0 : 6] || '#ffffff'), color: (color[isDark() ? 5 : 3] || '#000000') }">
			<div class="flex items-center gap-2">
				...
				<div v-if="course.type == 'Réunion'" class="flex-1 flex flex-col justify-center">
					<h2 class="text-lg font-bold">{{ maxScreen('xs') ? module.short : course.summary }}</h2>
				</div>
				<div v-else class="flex-1 flex flex-col justify-center">
					<h2 class="text-lg font-bold -mt-1 -mb-1.5" :style="{ color: color[3] }">{{ maxScreen('xs') ? module.short : course.summary }}</h2>
					<span class="text-sm font-semibold" :style="{ color: color[isDark() ? 4 : 3] }">{{ course.type }} - {{ course.module }}</span>
				</div>
				<div class="cursor-pointer flex items-center gap-1 bg-red-500 text-white text-sm font-semibold rounded-full p-1.5 duration-300 dark:bg-white/5 hover:scale-105" v-on:click="focusedCourse = null" :style="{ backgroundColor: (color[3] || '#000000') + '40' }"><CrossIcon class="w-3.5 h-3.5" :color="color[3] || '#000000'" /></div>
			</div>
			<div>
			...
			<div class="flex">
				...
				<div class="flex-1 -space-y-1">
					<p class="font-bold">{{ toFormatHHMM(new Date(course.start)) }}</p>
					<p class="text-sm">{{ toFormatHHMM(new Date(course.end)) }}</p>
				</div>
			</div>
